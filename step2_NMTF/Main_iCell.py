...
for root, dirs, files in os.walk('input'):
    for file in files:
        if file.endswith('.csv'):
            ...
            PPI, nodes, node2ind = nm.Load_Network(f'{MolNets_dir}/{cell_cond}/PPI_{case}_{cell_cond}.edgelist') # nodes = gene nodes, node2ind = gene node to index
            PPI_mat = nm.Make_Adj(PPI, nodes, node2ind, MOL_or_EXP[0])
            
            print('---- Loading COEX network')
            COEX = nx.read_edgelist(f'{MolNets_dir}/{cell_cond}/COEX_{case}_{cell_cond}.edgelist')
            COEX_mat = nm.Make_Adj(COEX, nodes, node2ind, MOL_or_EXP[0])
            
            print('---- Loading GI network')
            GI = nx.read_edgelist(f'{MolNets_dir}/{cell_cond}/GI_{case}_{cell_cond}.edgelist')
            GI_mat = nm.Make_Adj(GI, nodes, node2ind, MOL_or_EXP[0])
            
            print('---- Loading MI network')
            MI = nx.read_edgelist(f'{MolNets_dir}/{cell_cond}/MI_{case}_{cell_cond}.edgelist')
            ...
            for nets in nets_everything:
                ...
                OBJ_fig.savefig(k1k2_folder + '/' + nets + '_OBJ.png', dpi = 600)
                plt.close(OBJ_fig)
            ...
            #     else:
            #         nm.Save_Matrix_Factor(S_EXP, k1k2_folder  + '/'  + nets + '_S_onlyMOLs_noEXP.csv', featuresG1, featuresG2)	
